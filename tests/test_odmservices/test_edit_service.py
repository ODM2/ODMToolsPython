...
class TestSeriesService:
    ...
    def test_save_series(self):
        assert self.edit_service.save(self.series)

    def test_save_as_series(self):
        var = test_util.add_variable(self.session)
        print var
        assert self.edit_service.save_as(var= var.id)
        assert self.edit_service._series_service.series_exists(self.series.site.id, var.id, self.series.method.id,
                                                 self.series.source.id, self.series.qcl.id)

'''
    def test_save_as_existing_series(self):
        assert not self.series_service.save_existing(series)
'''
