...
class frmAddPoint(wx.Dialog):
    ...
    def _init_table(self, series=None):
        ...
        self.qual_choice_editor = wx.grid.GridCellChoiceEditor(["<None>"] + self.qualchoices.keys() + ["<Create New...>"],
                                                               False)
    ...
    def OnBtnSaveButton(self, event):
        ...
        for r in range(num_rows):
            ...
            for c in range(num_cols):
                if self.grdDataValues.GetCellValue(r,c) != u'':
                    row[c]=(self.grdDataValues.GetCellValue(r, c))
            # convert date fields to datetime objects
            row[2] = datetime.datetime.strptime(row[2], self.datetime_format)
            row[4] = datetime.datetime.strptime(row[4], self.datetime_format)
    ...
    def IsRowFilled(self, row):
        ...
        if (self.grdDataValues.GetCellValue(row, 0) != "" and  # Data Value
            ...
            try:
                ...
                datetime.datetime.strptime(local_datetime, self.datetime_format)
                datetime.datetime.strptime(utc_datetime, self.datetime_format)
